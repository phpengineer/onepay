...
class UserModel extends Model{
	...
	protected $_validate = array(
	    array('username', 'email', '请输入合法邮箱', self::EXISTS_VALIDATE, 'regex',self::MODEL_INSERT),
	    array('username', '', '用户邮箱已被注册', self::EXISTS_VALIDATE, 'unique',self::MODEL_INSERT),
	...
	public function info($id, $field = true){
		...
		return M('User')->field($field)->where($map)->find();
	}
	...
	public function password(){
        ...
        $this->id=UID;
        $res = $this->save();
        A('Api/UserApi')->User_edit(UID,I('post.oldpassword'),I('post.password'));
        return $res;
    }
	...
	public function records($id,$p,$state){
		$subQuery = M('shop_record')->field('pid')->distinct(true)->where('uid='.$id)->order('create_time desc')->select(false);
		if(is_numeric($state)){
			$subQuery.='and state='.$state;
		}
		$period = M('shop_period')->where('id in '.$subQuery)->field(true)->page($p.',20')->order('state asc,end_time desc')->select();
		if($period){
	...
	public function displays($p,$id,$num=20,$sid=''){
		...
		if($shared){
			foreach ($shared as $k=>$v){
				...
				$list[$k]['thumbpic']=explode(',',$v['thumbpic']);
    ...
    public function recharge_list($p,$paydate){
    	...
    	if($p){
    		$list=M('shop_order')->where('type>1 and uid='.UID.$where)->page($p.',20')->order('id desc')->select();
    	}else{
    		$list=M('shop_order')->where('type>1 and uid='.UID.$where)->order('id desc')->select();
    	}
    	if($list){
			foreach ($list as $k=>$v){
