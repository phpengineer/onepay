...
class ShopModel extends Model{
	...
	public function detail($id){
		$info=M('shop_period')->table('__SHOP__ shop,__SHOP_PERIOD__ period')->field('shop.id as sid,shop.category,shop.name,shop.cover_id,shop.price,shop.status,shop.content,shop.display,shop.meta_title,shop.keywords,shop.description,period.number,period.no,period.id')->where('shop.id=period.sid and period.id='.$id)->find();
	...
	public function period($p,$cid=0,$num=20,$order='shop.hits|desc'){
		if($cid){
			$category=' and shop.category='.$cid;
		}
		$order=str_replace('|',' ',$order);
		$shop=M('shop_period')->table('__SHOP__ shop,__SHOP_PERIOD__ period')->field('shop.id as sid,shop.name,shop.cover_id,shop.price,shop.position,period.number,period.no,period.id')->where('shop.id=period.sid and shop.display=1 and period.state=0'.$category)->page($p,$num)->order($order)->select();
		if($shop){
			foreach ($shop as $k=>$v){
	...
	public function over($id){
		...
		$attend=D('Shop')->user_num($period["uid"],$period["id"]);
