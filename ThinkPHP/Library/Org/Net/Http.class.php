...
class Http
{
	...
	//通过POST方式发送数据
	static public function doPost($url, $post_data=array(), $timeout=10,$header="") 
	{
	...
	//通过curl post数据
	static public function curlPost($url, $post_data=array(), $timeout=10,$header="") 
	{
		$header=empty($header)?"":$header;
	...
	//通过socket get数据
	static public function socketGet($url,$timeout=10,$header="")
	{
		$header=empty($header)?"":$header;
		$url2 = parse_url($url);
	...
	//通过socket post数据
	static public function socketPost($url, $post_data=array(), $timeout=10,$header="") 
	{
		$header=empty($header)?"":$header;
		$post_string = http_build_query($post_data);  
