...
class WechatAuth {
    ...
    private $fileApiURL = 'http://file.api.weixin.qq.com/cgi-bin';

    /**
    * 支付路径
    * @var string
    */
    private $payURL = 'https://api.mch.weixin.qq.com/pay';
    ...
    /**
     * 获取jsapi_ticket，用于后续接口访问
     * @return array jsapi_ticket信息，包含 ticket 和有效期
     */
	...
	/**
     * 上传永久图文资源
     * @param string $data    图文资源数组
     */
	public function materialAddNews($data){
		$data = array('articles'  => $data);
		return $this->api('material/add_news', $data, 'POST', '', true);
	}
     ...
     */
    public function mediaGetUpload($media_id){
        $param = array(
            'access_token' => $this->accessToken,
            'media_id'     => $media_id
        );
        return self::http($this->fileApiURL.'/media/get', $param);
    }
     ...
     * @return string           媒体资源下载地址
     */
    public function mediaGetDown($media_id){
        $param = array(
            'access_token' => $this->accessToken,
            'media_id'     => $media_id
        );
	...
	/**
     * 删除永久素材
     * @param string $media_id    资源media_id
     */
    ...
    public function mediaGet($media_id){
		...
		return self::http($this->apiURL.'/material/get_material', $param, $data, 'POST');
    }
	...
	public function updateNews($data){
		return $this->api('material/update_news', $data, 'POST', '', true);
	}
     ...
     * @return array           资源列表
     */
    public function mediaListGet($type,$offset,$count){
        $param = array('access_token' => $this->accessToken);
		$data = array(
			"type"=>$type,
			"offset"=>$offset,
        ...
        return $this->api('material/batchget_material', $data, 'POST', $param);
    }
     ...
     */
    public function messageSendall($filter,$content,$type){
		//基础数据
        $data = array(
            'filter'=>$filter,
			$type=>$content,
		...
		return $this->api('message/mass/sendall',$data,'POST',false);
	}
	...
	/**
     * 查询门店
     * @param  array $poi_id 门店id
     */
    public function getPoi($poi_id){
        $data = array("poi_id"=>$poi_id);
        return $this->api('poi/getpoi', $data);
    }
	
	/**
     * 查询门店列表
     * @param  array $begin 开始位置，0 即为从第一条开始查询
	 * @param  array $limit 返回数据条数，最大允许50，默认为20
     */
    public function getPoilist($begin,$limit){
        ...
        return $this->api('poi/getpoilist', $data);
    }
	
	/**
     * 上传门店图片
     * @param  array $buffer 图片文件的数据流
     */
    public function upLoadimg($buffer){
		$filename = realpath($buffer);
		$data = array("buffer"=>"@{$filename}");
        return $this->api('media/uploadimg', $data, 'POST', '', false);
    }
	
	/**
     * 创建门店
     * @param  array $business array数据
     */
    public function addPoi($business){
		$data = array("business"=>$business);
        return $this->api('poi/addpoi', $data);
    }
    ...
    public function updatePoi($business){
		$data = array("business"=>$business);
        return $this->api('poi/updatepoi', $data);
    }
	
	/**
     * 删除门店
     * @param  array $poi_id poi_id
     */
    public function delPoi($poi_id){
		$data = array("poi_id"=>$poi_id);
        return $this->api('poi/delpoi', $data);
    }
    ...
    /**************************************支付模块***********************************/
	function postXmlSSLCurl($url,$xml,$second=300){
		$ch = curl_init();
		...
		curl_setopt($ch,CURLOPT_SSLCERT, getcwd().'/cacert/apiclient_cert.pem');
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, getcwd().'/cacert/apiclient_key.pem');
    ...
    public function unifiedOrder($data){
        ...
        //$xml=$this->http($this->payURL.'/unifiedorder', '', $data, 'POST');
        $xml=$this->postXmlSSLCurl($this->payURL.'/unifiedorder',$data);
        return $this->FromXml($xml);
    }

    public function sendhonbao($data,$key){
    	$data['nonce_str']=$this->getNonceStr();//随机字符串
    	$data['wxappid']=$this->appId;
    	$data['client_ip']=get_client_ip();
    	$data['sign']=$this->MakeSign($data,$key);
    	$data=$this->ToXml($data);
    	$xml=$this->postXmlSSLCurl('https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack',$data);
        return $this->FromXml($xml);
    }
    ...
    public function CheckSign($data,$key){
        ...
        if($this->MakeSign($data,$key)!=$data['sign']){
            return false;
        }
        return true;
    }
