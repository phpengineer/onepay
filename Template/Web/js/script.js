$(function(){
	    ...
	    $.getJSON($('#shoplist').attr('url'), {p: page,order:$('.inline.active').attr('order')}, function(json) {
	    	if (json) {
		        ...
		        for (var i = 0; i < json.length; i++) {
					...
					str += '<div class="reco-ls huise">总需：' + json[i].price + ' 人次</div>';
					...
					str += '<div class="reco-bottom"><div class="ro-goods">';
					str += '我要参与：<div class="ro-goods-inputs">';
					str += '<input value="' + json[i].pid + '" name="pid" type="hidden">';
					str += '<input type="type" value="1" surplus="' + json[i].surplus + '" name="number">';
	...
	function announced_ajax(obg){
		$.getJSON($('#announced').attr('url'),{
			...
			if (json) {
				switch (htmltype) {
		        case 'announced':
		          	var str = '<div class="user-img"><a href="'+ThinkPHP.U("user/user",{"id":json[i].uid}) + '"><img src="' + json[0].user_pic + '"></a></div>';
		            str += '<div class="user-info">';
		            str += '<span>恭喜 <a href="'+ThinkPHP.U("user/user",{"id":json[i].uid}) + '" class="a3">' + json[0].user + '</a></span>';
		        ...
		        default:
					...
					str += '<li>获奖者：<a href="'+ThinkPHP.U("user/user",{"id":json[i].uid}) + '" class="a3">' + json[0].user + '</a></li>';
 ...
 //  	});
    ...
    $('body').on('click','.ro-jia',function(){
        ...
        if(num>1){
          $(this).siblings('input[name="number"]').val(num-1);
        }
    });
    ...
    $('body').on('submit','form',function(){
        var surplus = $(this).find('input[name="number"]').attr('surplus');
